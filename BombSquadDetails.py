...
default_config = {'Leave The Server Directly': False,
                  ...
                  'Quick Game Language 1': ba.app.lang.default_language,
                  ...
                  'Target Trans Lang': ba.app.lang.default_language}
...
translate_languages = {'Auto Detect': 'auto', 'Arabic': 'ar', 'Croatian': 'hr', 'Czech': 'cs',
                       ...
                       'Korean': 'ko', 'Malay': 'ms', 'Persian': 'fa', 'Polish': 'pl',
...
aviable_game_languages = ba.app.lang.available_languages
    ...
    def _set_tab(self, road_is_open, tab_id):
        ...
        if tab_id == 'main_tab':
            ...
            PopupMenu(parent=self._subcontainer,
                      position=(0, 0),
                      autoselect=False,
                      on_value_change_call=ba.Call(self._set_language, 'Quick Game Language 1'),
            ...
            PopupMenu(parent=self._subcontainer,
                      position=(0, 0),
                      autoselect=False,
                      on_value_change_call=ba.Call(self._set_language, 'Quick Game Language 2'),
                      choices=aviable_game_languages,
                      button_size=(275, 75),
...
class LocalAccountManagerWindow(ba.Window):
    ...
    def _add_account(self):
        if _bainternal.get_v1_account_state() != 'signed_in':
            show_sign_in_prompt()
            return
        elif _bainternal.get_v1_account_type() != 'Local':
            ba.screenmessage(message='You cannot add a V2 account! Please sign in with a local account.',
                             color=(1,0,0))
            ba.playsound(ba.getsound('error'))
            return

        try:
            account_ds = _bainternal.get_v1_account_display_string()
    ...
    def _show_accounts_folder(self):
        if platform == 'windows': os.startfile(os.path.realpath(self._accounts_folder_path))
        else: os.system('open "%s"' % self._accounts_folder_path)
...
class NewMainMenu:
    ...
    def _new_confirm_leave_party(self):
        global in_server
        in_server = False
        ba.internal.disconnect_from_host() if config['Leave The Server Directly'] else self._old_confirm_leave_party()
    ...
    def quick_language():
        if config['Language Order Is 1']:
            ba.app.lang.setlanguage(config['Quick Game Language 2'])
            config['Language Order Is 1'] = False
        else:
            ba.app.lang.setlanguage(config['Quick Game Language 1'])
    ...
    def reconnect_server():
        if _ba.get_connection_to_host_info().get('name', False):
            ...
            ba.timer(0.1, ba.Call(ba.internal.connect_to_party, server_ip, server_port, _print_progress))
        else:
            ba.screenmessage('You are not on a server', color=(1,0,0))
            ba.playsound(ba.getsound('error'))
...
class NewPluginWindow:
    ...
    def _new_init(self, transition='in_right'):
        ...
        uiscale = ba.app.ui.uiscale
        delete_button_x = 610 if uiscale is ba.UIScale.SMALL else 510
        ...
        ba.buttonwidget(edit=self._back_button, on_activate_call=ba.Call(back_to_allsettings_window, self._root_widget, AdvancedSettingsWindow))
...
class NewAccountSettingsWindow:
    ...
    def _new_init(self, transition: str = 'in_right', modal: bool = False,
        ...
        pb_id = _bainternal.get_v1_account_misc_read_val_2('resolvedAccountID', '')
        if _bainternal.get_v1_account_state() == 'signed_in':
            ba.textwidget(parent=self._subcontainer,
...
class NewPartyWindow:
    ...
    def _new_init(self, origin: Sequence[float] = (0, 0)):
        ...
        self._switch_players_button = ba.buttonwidget(
            parent=self._root_widget,
            scale=0.7,
            position=(0, self._height-85),
            size=(50, 50),
            icon=ba.gettexture('replayIcon'),
            iconscale=1.17,
            on_activate_call=ba.Call(switch_players, self._update))
    ...
    def _new_add_msg(self, msg: str):
        txt = ba.textwidget(parent=self._columnwidget,
                            ...
                            selectable=True,
                            click_activate=True)
        ba.textwidget(edit=txt, on_activate_call=ba.Call(OnMessageClickWindow, txt))
        if 'https://' in msg:
            split_msg = msg.split(' ')
            for i in split_msg:
...
class TranslateWindow:
    def __init__(self):
        self._uiscale = ba.app.ui.uiscale
        self._translated_once = False

        self._root_widget = ba.containerwidget(parent=_ba.get_special_widget('overlay_stack'),
                                               size=(450, 250),
                                               transition='in_scale',
                                               scale=(1.6 if self._uiscale is ba.UIScale.SMALL else
                                                      1.2 if self._uiscale is ba.UIScale.MEDIUM else 1.1),
                                               on_outside_click_call=ba.Call(self._back, sound=True))
        ...
        self._source_button = PopupMenu(parent=self._root_widget,
                                        position=(50, 140),
                                        autoselect=False,
                                        on_value_change_call=ba.Call(self._set_translate_language, 'Source Trans Lang'),
        ...
        self._target_button = PopupMenu(parent=self._root_widget,
                                        position=(243, 140),
                                        autoselect=False,
                                        on_value_change_call=ba.Call(self._set_translate_language, 'Target Trans Lang'),
                                        choices=aviable_translate_languages[1:],
                                        button_size=(150, 30),
                                        current_choice=config['Target Trans Lang'])

        self._source_trans_text_field = ba.textwidget(parent=self._root_widget,
                                                      editable=True,
                                                      size=(530, 40),
                                                      position=(40, 100),
                                                      text='',
        ...
        self._result_text = ba.textwidget(parent=self._root_widget,
                                          size=(530, 40),
                                          position=(40, 67),
        ...
        self._send_button = ba.buttonwidget(parent=self._root_widget,
                                            size=(150, 30),
                                            label='Send',
                                            button_type='square',
                                            autoselect=True,
        ...
        ba.textwidget(edit=self._source_trans_text_field, on_return_press_call=self._translate_button.activate)
    ...
    def _start_translating(self):
        self._text = ba.textwidget(query=self._source_trans_text_field)
        ba.screenmessage('Translating...', color=(0,1,0))
    ...
    def _translate(self):
        self._translated_text = translate(source=translate_languages[config['Source Trans Lang']],
                                          ...
                                          text=self._text)
        ba.pushcall(self._show_result, from_other_thread=True)

    def _show_result(self):
        ba.textwidget(edit=self._result_text, text=self._translated_text)
        if not self._translated_once: self._translated_once = True

    def _send_translated_text(self):
        if self._translated_once: _ba.chatmessage(ba.textwidget(query=self._result_text))
        else: ba.playsound(ba.getsound('error'))
...
class OnMessageClickWindow:
    def __init__(self, textwidget):
        ...
        self._message = ba.textwidget(query=textwidget)
        self._uiscale = ba.app.ui.uiscale
        ...
        self._root_widget = ba.containerwidget(parent=_ba.get_special_widget('overlay_stack'),
                                               ...
                                               scale=(1.6 if self._uiscale is ba.UIScale.SMALL else
                                                      1.2 if self._uiscale is ba.UIScale.MEDIUM else 1.1),
                                               on_outside_click_call=ba.Call(self._close_with_sound))
        ...
        self._translate_message_button = ba.buttonwidget(parent=self._root_widget,
                                                         color=(0,1,1),
                                                         button_type='square',
                                                         label='Translate',
                                                         size=(130, 110),
        ...
        self._copy_message_button = ba.buttonwidget(parent=self._root_widget,
                                                    color=(0.30,0.72,0),
                                                    button_type='square',
                                                    label='Copy',
                                                    size=(130, 110),
                                                    position=(309, 50),
                                                    on_activate_call=ba.Call(self._confirm_copy_message))

        if config['Copy Message Directly']:
            ba.buttonwidget(edit=self._copy_message_button, on_activate_call=ba.Call(self._copy_message))

    def _confirm_copy_message(self):
        ba.playsound(ba.getsound('swish'))
        ConfirmWindow(text='Do you want to copy this message?',
                      action=ba.Call(self._copy_message),
                      cancel_text='No', ok_text='Yes')
    ...
    def _translate(self):
        ...
        self._translated_msg = translate(source=translate_languages[config['Source Trans Lang']],
                                         target=translate_languages[config['Target Trans Lang']],
                                         text=msg)
        ba.pushcall(self._edit_chat_message, from_other_thread=True)

    def _edit_chat_message(self):
        ba.textwidget(edit=self._textwidget, text=self._nickname+self._translated_msg)
...
class TranslateChatMessageWindow:
    def __init__(self, root_widget, textwidget):
        ...
        ba.containerwidget(edit=self._root_widget, on_outside_click_call=ba.Call(self._back, sound=True))
        ...
        try:
            ...
            for i in self._history:
                ...
                ba.textwidget(parent=self._subcontainer,
                              ...
                              selectable=True,
                              click_activate=True,
                              on_activate_call=ba.Call(ba.internal.connect_to_party,
...
old_connect_to_party = ba.internal.connect_to_party
...
ba.internal.connect_to_party = new_connect_to_party

old_disconnect_from_host = ba.internal.disconnect_from_host
...
ba.internal.disconnect_from_host = _new_disconnect_from_host

def run_ping_thread():
    if _ba.get_connection_to_host_info().get('name', False): PingThread(server_ip, server_port).start()
    else:
        ba.screenmessage('You are not on a server', color=(1,0,0))
        ba.playsound(ba.getsound('error'))
...
def check_ping(ping):
    if _ba.get_connection_to_host_info().get('name', False):
        ...
        if config['Share Ping In Chat']:
            _ba.chatmessage(config['Ping Message'].replace('!', short_ping))
        else:
