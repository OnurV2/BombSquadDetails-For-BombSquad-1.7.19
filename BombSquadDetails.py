...
class NewPluginWindow:
    ...
    def _new_init(self, transition='in_right'):
        ...
        self._delete_button = ba.buttonwidget(parent=self._root_widget,
                                              size=(40, 40),
                                              label='',
                                              position=(self._height/1.02, self._height-h-2),
                                              on_activate_call=ba.Call(show_delete_window))

        ba.imagewidget(
            parent=self._root_widget,
            position=(self._height/1.012, self._height-h-1.3),
            size=(33, 33),
            texture=ba.gettexture('textClearButton'),
        )
...
class NewPartyWindow:
    global confirm_copy_message, copy_message, confirm_open_url, vote_kick_player, add_friend_window, add_friend, switch_players
    ...
    def copy_message(message):
        if len(message.split(':')) > 1: msg = ':'.join(message.split(':')[1:])[1:]
